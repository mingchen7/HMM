...
evalAlpha = function(obs,pi,mu,sigma,A,i)
{
  ...
  else
  {
    ...
    alpha.pre = evalAlpha(obs,pi,mu,sigma,A,i-1);
...
evalBeta = function(obs,mu,sigma,A,i)
{
  beta.crt = c(0);
  D = length(mu);
  if(i == D)
  ...
  else
  {
    ...
    beta.nxt = evalBeta(obs,mu,sigma,A,i+1);
...
evalKsi = function(obs,mu,sigma,A,i,alpha.pre,beta.crt)
{
  D = length(mu);
...
#     alpha.pre = evalAlpha(obs,pi,mu,sigma,A,i-1);
#     beta.crt = evalBeta(obs,mu,sigma,A,i);
    ...
    for(j in 1:K.pre)
    {
      for(k in 1:K)
      {
        ...
        ksi[j,k] = alpha.pre[j] * prob * A.crt[j,k] * beta.crt[k];                
      }
    }
...
HiddenMarkovModel = function(data,mu.init,sigma.init,pi.init,A.init)
{
  ...
  for(iter in 1:100)
  {
    ...
    for(n in 1:N)
    {
      ...
      {    
        #Evaluate alpha, beta
        alpha[[i]][n, ] = evalAlpha(obs,pi,mu,sigma,A,i);
        beta[[i]][n, ]= evalBeta(obs,mu,sigma,A,i);                
        ...
        if(i >= 2)
        {
          ksi[[i-1]][,,n] = evalKsi(obs,mu,sigma,A,i,alpha[[i-1]][n,],beta[[i]][n,]);  
        }        
      }
    }
    ...
    for(n in 1:N)
    { 
      # evaluate p(Xn) by setting i to D
      tmp = sum(alpha[[D]][n,]);
      LogLikelihood[iter] = LogLikelihood[iter] + log(tmp);            
    }
